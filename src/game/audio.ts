export class FilthyAudio{
 ...
 start(){if(this.ctx){void this.ctx.resume();return}this.ctx=new AudioContext();this.master=this.ctx.createGain();this.master.gain.value=this.muted?0:.18;this.master.connect(this.ctx.destination);this.beat=window.setInterval(()=>this.tick(),240);void this.ctx.resume()}
 ...
 pause(paused:boolean){if(!this.ctx)return;void(paused?this.ctx.suspend():this.ctx.resume())}
 stop(){if(this.beat)clearInterval(this.beat)}
}
